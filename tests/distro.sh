#!/usr/bin/env bash
# Distro tests for ddlc-hyprlock: run install.sh for real, as root, inside a container
# of an actual distribution — the one thing tests/run.sh cannot do. Here the real
# package manager provides the engine's tools, by running the very commands the
# preflight printed when it refused. hyprlock itself is a session dependency the
# preflight only warns about — a theme installs fine on a machine that has not
# installed its compositor yet, and the suite drives the engine against stubs.
#
...
smoke() { # runs inside the container after a successful install
  local prefix="$1"
  local conf="$prefix/share/ddlc-hyprlock/hyprlock.conf"
  "$prefix/bin/ddlc-hyprlock" --version | grep -qxF "ddlc-hyprlock $(cat VERSION)"
  # The rendered config names its runtime paths, with no staging token left behind
  if grep -q '@share@' "$conf"; then return 1; fi
  grep -qF "$prefix/share/ddlc-hyprlock/just-monika.png" "$conf"
  # The glitch shader ships — the hyprctl flash mode reads it from beside the assets
  test -f "$prefix/share/ddlc-hyprlock/glitch.frag"
  # The whole behaviour suite against the installed engine, locker and journal stubbed
  DDLC_HYPRLOCK_BIN="$prefix/bin/ddlc-hyprlock" ./tests/run.sh >/dev/null
}
...
prefix=/usr/local
bin_path="$prefix/bin/ddlc-hyprlock"
...
if ((rc != 0)); then
  ...
  commands=$(printf '%s\n' "$out" | sed -n 's/^  \$ //p')
  if [[ -z "$commands" ]]; then
    echo "::notice title=ddlc-hyprlock distro test::SKIP on $distro — guidance is manual-only"
...
say "the installed engine answers"
